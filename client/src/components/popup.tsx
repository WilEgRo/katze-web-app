...
const PopupReglas: React.FC<PopupProps> = ({ onClose }) => {
    const [aceptado, setAceptado] = useState(false);

    // Lista de reglas con iconos para mejor UX
    const reglas = [
        {
        ...
        },
        {
            ...
            desc: "Si vives en departamento, es obligatorio tener mallas de seguridad en ventanas. No permitimos gatos con acceso libre a la calle.",
            icon: <FaShieldAlt className="text-blue-500" />
        },
        {
            titulo: "Solvencia Económica",
            desc: "Compromiso de vacunas anuales, alimento de calidad y atención veterinaria cuando se requiera.",
    ...
    return (
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm z-[9999] p-4 animate-fade-in">
        
        {/* Contenedor del Modal */}
        <div className="bg-white dark:bg-katze-dark-card rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-100 dark:border-gray-700 flex flex-col max-h-[90vh]">
            ...
            <div className="bg-katze-gold p-6 flex justify-between items-center text-white">
                ...
                <Link to="/" onClick={onClose} className="hover:bg-white/20 p-2 rounded-full transition">
                    <FaTimes size={20} />
                </Link>
            </div>
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar">
                ...
                <div className="space-y-4">
                    {reglas.map((regla, index) => (
                        <div key={index} className="flex gap-4 p-4 bg-gray-50 dark:bg-black/20 rounded-xl border border-gray-100 dark:border-gray-800">
                            <div className="mt-1 text-xl">{regla.icon}</div>
                            <div>
            ...
            <label className="flex items-center gap-3 cursor-pointer group mb-6 select-none">
                <div className={`w-6 h-6 border-2 rounded flex items-center justify-center transition-colors ${aceptado ? 'bg-katze-gold border-katze-gold' : 'border-gray-400 bg-white dark:bg-transparent'}`}>
                ...
                <span className="text-sm text-gray-600 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white transition">
                    He leído y acepto los requisitos de adopción.
                </span>
            </label>
            ...
            <div className="flex gap-3">
                <Link
                    ...
                    className="flex-1 px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 
                     text-gray-600 dark:text-gray-300 font-bold hover:bg-gray-100 dark:hover:bg-gray-700 
                ...
                <Link
                    ...
                    onClick={(e) => {
                        if (!aceptado) {
                            e.preventDefault(); // 1. Si no aceptó, BLOQUEAMOS el click (no navega)
                        } else {
                            onClose(); // 2. Si aceptó, CERRAMOS el modal y dejamos que navegue
                        }
                    }}
                    className={`flex-1 px-4 py-3 rounded-xl font-bold text-white transition-all shadow-lg flex items-center justify-center gap-2
                        ${aceptado 
                        ? 'bg-green-600 hover:bg-green-700 cursor-pointer transform hover:-translate-y-1' 
                        : 'bg-gray-300 dark:bg-gray-700 cursor-not-allowed opacity-70'
                        }
                    `}
                >
                    {aceptado ? 'Continuar a la adopción' : 'Lee y Acepta para Continuar'}
                </Link>
          </div>
        </div>
